...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  try {
    ...
    res.status(200).json({ ok: true, data: data.fastestModel });
  } catch (err: unknown) {
...
Never commit secrets. Add variables via the Vercel CLI, then read them in your handler and pass them to the Stagehand constructor:
