...
Without variables you'd accumulate a separate cache miss for every unique email address you type. With variables you prime the cache once and hit it forever. The value also never reaches the model: Stagehand sends the variable name and substitutes the real value into the resolved action right before it runs.

<Warning>
The value does reach the cache service. Variables travel with the rest of the request payload when caching is on, so pass `cache: false` on any call that carries a credential. See [prompting with variables](/v4/best-practices/prompting-best-practices) for the full caveat.
</Warning>

{/* TODO: verify the claim above that the cache key is derived from variable keys rather than values. buildActCacheData transmits the whole options.variables record, and the comment says only key-participating fields are sent, which would mean differing values produce differing keys and break the sharing this accordion promises. */}
