...
async def plan_checkout_workflow():
    ...
    checkout_button = next(
        (a for a in cart_actions if "checkout" in a.description), None
    )
    ...
    # Step 3: Plan execution order
    shipping_fields = [a for a in checkout_actions if "shipping" in a.description]
    payment_fields = [a for a in checkout_actions if "payment" in a.description]
    submit_button = next(
        (a for a in checkout_actions if "complete order" in a.description), None
    )
...
func planCheckoutWorkflow(
	...
	for _, action := range cartActions.Data {
		if strings.Contains(action.Description, "checkout") {
	...
	for i, action := range checkoutActions.Data {
		switch {
		case strings.Contains(action.Description, "shipping"):
			plan.ShippingFields = append(plan.ShippingFields, action)
		case strings.Contains(action.Description, "payment"):
			plan.PaymentFields = append(plan.PaymentFields, action)
		case strings.Contains(action.Description, "complete order"):
