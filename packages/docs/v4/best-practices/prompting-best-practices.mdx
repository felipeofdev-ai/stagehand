...
Variables keep sensitive information out of prompts and logs. Stagehand shows the model only the variable name and substitutes the value locally, so the secret never leaves your process and never lands in a cache entry.
